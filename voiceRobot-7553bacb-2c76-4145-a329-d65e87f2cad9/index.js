...
function onIntent(intentRequest, session ) {                  //, callback) {
    ...
    if ("RobotIntent" === intentName) {
        ...
        var directionSlot = intent.slots.Direction,
			...
			introduceSlot = intent.slots.Introduction,
			turnValue,
			directionValue,
			speedValue,
			stopValue,
		...
		if (turnSlot && turnSlot.value) { 
			...
			var sessionAttributes = {};
			var shouldEndSession = false;
			var speechOutput = "You know I've got the sick moves! " + repromptText;
			//var shouldEndSession = true;
 			ctx.succeed(buildResponse(sessionAttributes, buildSpeechletResponse(cardTitle, speechOutput, repromptText, shouldEndSession)));
// 		    //client.end();
		    });  
        ...
        }else if (introduceSlot && introduceSlot.value) {
			introduceValue = introduceSlot.value.toLowerCase();
			
		    client.publish('JonasAlbert/vbot', introduceValue, function() {
		    var cardTitle = "Introducing";
			var repromptText = "What's next?";
			var sessionAttributes = {};
			var shouldEndSession = false;
			var speechOutput = "";
...
function getWelcomeResponse() {
    ...
    var speechOutput = "Yo, what's up? My name is MC nugget. How can I help?";
...
function buildSpeechletResponse(title, output, repromptText, shouldEndSession) {
    if (title == "Welcome" || title == "Turning" || title == "Going" || title == "Setting Speed" || title == "Stop" || title == "Session Ended") {
        return {
            outputSpeech: {
                type: "PlainText",
                text: output
    ...
    else if (title == "Introducing") {
        return {
            outputSpeech: {
                type: "SSML",
                ssml: '<speak> <break time="3s"/> "Hi guys! As I said before, I\'m MC Nugget, and I am the product of Jonas and Albert\'s infinite wisdom and brilliance. I\'m a robot that can be controlled by giving commands to auntie Alexa. I can move forward or backward, turn right or left, and dance. Now listen up as my old folks, Jonas and Albert, talk about my unique origins and amazing potential!" </speak>'
	        },
            reprompt: {
                outputSpeech: {
                    type: "PlainText",
        ...
        return {
            outputSpeech: {
                type: "SSML",
                ssml: '<speak>"You know I\'ve got the moves baby!" <audio src="https://s3.amazonaws.com/voicerobotmusic/EncodedMichaelJacksonBlackorWhite.mp3" /> "What\'s next?"</speak>'
	        },
            reprompt: {
                outputSpeech: {
                    type: "PlainText",
